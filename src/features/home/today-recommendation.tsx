...
function findDrinkById(id: string): Drink {
  const found = drinks.find((drink) => drink.id === id);
  if (!found) throw new Error(`Drink not found: ${id}`);
  return found;
}
...
function getWeatherBasedRecommendation(summary: string) {
  ...
  if (summary.includes("선선한 날씨") || summary.includes("쌀쌀한 날씨")) {
    return {
      drink: findDrinkById("moscato-dasti"),
      reason:
